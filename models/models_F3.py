...
class Generator(torch.nn.Module):
    def __init__(self, channels=[64, 64, 64, 64, 64, 64, 64], bottleneck_dim=32):
    ...
    def forward(self, x):
        ...
        x = torch.istft(torch.view_as_complex(x.float()), n_fft=640, hop_length=320, win_length=640, window=self.window.to(x.device), center=True, normalized=False, onesided=None, length=None, return_complex=False)
...
if __name__=='__main__':
    ...
    emb = encoder(input)
    print(emb.shape)
    emb = torch.randn(3, bottleneck_dim, 401, 32)
    out = decoder(emb)
    print(out.shape)
