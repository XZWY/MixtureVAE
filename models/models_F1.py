...
class Encoder(torch.nn.Module):
    def __init__(self, channels=[2, 64, 64, 64, 64, 128, 128], bottleneck_dim=32):
        ...
        t_kernel_size = [3,4,3,3,3,3]
        t_stride_size = [2,2,1,1,1,1]
        t_padding = [0,0,1,1,1,1]
        ...
        for c_idx in range(self.num_layers):
            conv_list.append(
                ...
                nn.Conv2d(channels[c_idx], channels[c_idx+1], (t_kernel_size[c_idx], f_kernel_size[c_idx]), stride=(t_stride_size[c_idx], f_stride_size[c_idx]), padding=(t_padding[c_idx],0)),
            )
...
class Generator(torch.nn.Module):
    def __init__(self, channels=[64, 64, 64, 64, 64, 128, 128], bottleneck_dim=32):
        ...
        for c_idx in range(self.num_layers):
            ...
            conv_list.append(
                nn.ConvTranspose2d(channels[self.num_layers-c_idx], channels[self.num_layers-c_idx-1], (t_kernel_size[self.num_layers-c_idx-1], f_kernel_size[self.num_layers-c_idx-1]), stride=(t_stride_size[self.num_layers-c_idx-1], f_stride_size[self.num_layers-c_idx-1]), padding=(t_padding[self.num_layers-c_idx-1],0)),
            )
            for j, (k, d) in enumerate(
                zip(
                    list(reversed(resblock_kernel_sizes)),
    ...
    def forward(self, x):
        '''
        x: bs, bottle_neck, T, 6
        out: bs, 
        '''
        bs, _, n_frames, _ = x.shape
        # x = self.conv_pre(x)
...
if __name__=='__main__':
    bottleneck_dim=32
    encoder = Encoder(bottleneck_dim=bottleneck_dim)
    decoder = Generator(bottleneck_dim=bottleneck_dim)
    input = torch.randn(3, 1, 16000)
    emb = encoder(input)
    print(emb.shape)
    emb = torch.randn(3, bottleneck_dim, 24, 32)
    out = decoder(emb)
    print(out.shape)
