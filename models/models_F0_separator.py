...
class MixtureEncoder(torch.nn.Module):
    def __init__(self, channels=[2, 64, 64, 128, 128, 256, 512], bottleneck_dim=32):
        ...
        self.conv_post = weight_norm(nn.Conv2d(channels[-1], 4*bottleneck_dim*2, 1, 1, padding=0))
    ...
    def forward(self, audio):
        ...
        x = self.conv_post(x) # bs, 4*bottleneck*2, nframes
        bs, _, n_frames, n_freqs = x.shape

        x = x.view(bs, 4, self.bottleneck_dim*2, n_frames, n_freqs)
...
if __name__=='__main__':
    ...
    encoder = MixtureEncoder(channels=[2, 64, 128, 256, 256, 256, 256], bottleneck_dim=bottleneck_dim).cuda()
