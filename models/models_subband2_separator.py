...
class MixtureEncoder(torch.nn.Module):
    def __init__(self, channels=[2, 64, 64, 64, 64, 128, 128], bottleneck_dim=32):
        ...
        for ch in channels[1:]:
            self.embs_mu.append(nn.Parameter(torch.zeros(4, ch, 1, 1)))
            self.embs_sigma.append(nn.Parameter(torch.ones(4, ch, 1, 1)))
        self.embs_mu = nn.ParameterList(self.embs_mu)
        self.embs_sigma = nn.ParameterList(self.embs_sigma)
    ...
    def forward(self, audio, indices):
        '''
        x: bs, 2, T, F
        indices, bs
        ...
        for i in range(self.num_layers):
            ...
            indices_expand = indices.expand(-1, ch, -1, -1)
            current_mu = torch.gather(self.embs_mu[i], 0, indices_expand)
            current_sigma = torch.gather(self.embs_sigma[i], 0, indices_expand)
            x = current_sigma * x + current_mu
...
if __name__=='__main__':
    ...
    input = torch.randn(4, 1, 16000).cuda()
    indices = torch.tensor([0,1,2,3]).cuda()
    emb = encoder(input, indices)
    print(emb.shape)
