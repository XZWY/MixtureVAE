...
def train(rank, a, h):
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # vanilla VAE loss
            loss_kl = batch['loss_KLD'].mean()*1000
            loss_l2_spec = batch['loss_reconstruction'].mean()*50
            loss_l1 = batch['loss_l1']
            loss_snr = batch['loss_snr'].mean()
            # loss_gen_all = loss_kl + loss_l2_spec
            loss_gen_all = loss_snr
            # L1 Mel-Spectrogram Loss (non-vanilla)
            if steps < a.vanilla_steps and h.codec_loss:
                ...
                loss_non_vanilla = loss_gen_s + loss_gen_f + loss_gen_stft + loss_fm_s + loss_fm_f + loss_fm_stft + loss_mel
                loss_gen_all += loss_non_vanilla
            ...
            if rank == 0:
                ...
                if steps % a.stdout_interval == 0:
                    ...
                    #            time.time() - start_b))
                    print(
                        'Steps : {:d}, Gen Loss Total : {:4.3f}, loss snr : {:4.3f}, s/b : {:4.3f}'.
                        format(steps, loss_gen_all.item(), loss_snr.item(),
                               time.time() - start_b))
                # checkpointing
                ...
                if steps % a.summary_interval == 0:
                    sw.add_scalar("training/gen_loss_total", loss_gen_all,
                                  steps)
                    sw.add_scalar("training/loss_snr", loss_snr,
                                  steps)
                    # sw.add_scalar("training/mel_spec_error", mel_error, steps)
                    
                    # manually check training
                    if steps > 10:
                        sw.add_audio('gt_tr/y_{}'.format(steps%10), y[0],
                        ...
                        y_spec = mel_spectrogram(
                            ...
                            h.fmin, h.fmax)
                        sw.add_figure(
                            'generated_tr/y_spec_{}'.format(steps%10),
                            plot_spectrogram(
                                y_spec.squeeze(0).cpu().numpy()),
                            steps)

                        sw.add_audio('generated_tr/y_hat_{}'.format(steps%10),
                        ...
                        y_hat_spec = mel_spectrogram(
                            ...
                            h.fmin, h.fmax)
                        sw.add_figure(
                            'generated_tr/y_hat_spec_{}'.format(steps%10),
                            plot_spectrogram(
                                y_hat_spec.squeeze(0).detach().cpu().numpy()),
                            steps)
                ...
                if steps % a.validation_interval == 0 and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            val_err_tot += batch['loss_snr'].mean().item()
