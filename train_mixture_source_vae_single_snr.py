...
def train(rank, a, h):
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss_snr_mix = 10 * batch['loss_snr_mixture'].mean()
            loss_snr_source = 10 * batch['loss_snr_source'].mean()
            # loss_l1 = 10 * batch['loss_l1']
            loss_gen_all = loss_gen_s + loss_gen_f + loss_gen_stft + loss_fm_s + loss_fm_f + loss_fm_stft + loss_mel + loss_kl_source + loss_posterior_matching + loss_snr_mix + loss_snr_source
            ...
            if rank == 0:
                ...
                if steps % a.stdout_interval == 0:
                    ...
                    print(
                        'Steps : {:d}, Gen Loss Total : {:4.3f}, loss kl source: {:4.3f}, loss kl posterior: {:4.3f}, snr mix: {:4.3f},  snr source: {:4.3f},mel_error_source: {:4.3f}, mel_error_mixture: {:4.3f}, s/b : {:4.3f}'.
                        format(steps, loss_gen_all.item(), loss_kl_source.item(), loss_posterior_matching.item(), -loss_snr_mix.item(), -loss_snr_source.item(), mel_error_source, mel_error_mixture, time.time() - start_b))
                # checkpointing
                ...
                # if True:
                    ...
                    sw.add_scalar("training/snr mixture", loss_snr_mix, steps)
                    sw.add_scalar("training/snr source", loss_snr_source, steps)
                ...
                # if steps % 10 == 0 and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            val_mix_snr_tot += batch['loss_snr_mixture'].clone().mean().item()
                            val_source_snr_tot += batch['loss_snr_source'].clone().mean().item()
                        ...
                        val_snr_mix = val_mix_snr_tot / (j + 1)
                        val_snr_source = val_source_snr_tot / (j + 1)
                        ...
                        sw.add_scalar("validation/snr mixture", val_snr_mix, steps)
                        sw.add_scalar("validation/snr source", val_snr_source, steps)
                        if not plot_gt_once:
                            plot_gt_once = True
