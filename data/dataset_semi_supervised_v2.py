...
class dataset_semi_supervised_v2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for source_type in self.source_types:
            source_batch = self.ds_musdb[source_type][musdb_idx]
            
            for key in source_batch.keys():
                if key == 'mixture':
                    batch[source_type+'_mixture'] = source_batch['mixture']
                else:
                    batch[key] = source_batch[key]
        batch['mixture_fma'] = mixture_fma
        return batch
        

def collate_func_semi_supsevised_v2(batches):
    '''
...
if __name__=='__main__':
    ...
    dataset = dataset_semi_supervised_v2(
    ...
    batch = collate_func_semi_supsevised_v2([dataset[0], dataset[1], dataset[2]])
    print('batch')
    print(batch.keys())
    # batch['mixture]: bs, T
    # batchp['vocals]: bs, T
