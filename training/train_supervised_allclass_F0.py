...
def train(rank, a, h):
    ...
    # ckpt_source = torch.load(os.path.join(h.sourcevae_ckpt_dir, 'ckpt_sourcevae'))
    mixtureVAE = MixtureVAE(h, load_model=False, model_ckpt=None, load_source_vae=False, sourcevae_ckpt=None).to(device)
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            y_source = batch['output_source'] # bs, 1, T
            y_mix = batch['output_mix'] # bs, 1, T
            bs = y_source.shape[0]

            y = batch['reference'] # bs, 1, T
            ...
            loss_kl_source = h.lambda_kl * batch['loss_KLD'].mean()
            loss_posterior_matching = h.lambda_posterior * batch['loss_posterior_matching'].mean()
            # loss_l1 = 10 * batch['loss_l1']
            loss_snr_rec = batch['loss_snr_source'].mean()
            loss_snr_sep = 20 * batch['loss_snr_mixture'].mean()
            if h.posterior_only:
                loss_gen_all = loss_gen_stft + loss_fm_stft + loss_mel + loss_kl_source + loss_posterior_matching + loss_snr_rec
            else:
                loss_gen_all = loss_gen_stft + loss_fm_stft + loss_mel + loss_kl_source + loss_posterior_matching + loss_snr_rec + loss_snr_sep
            ...
            if rank == 0:
                ...
                if steps % a.stdout_interval == 0:
                    with torch.no_grad():
                        ...
                        y_h_hat_mel_mix = mel_spectrogram(
                                y_mix.squeeze(1), h.n_fft, h.num_mels, h.sampling_rate,
                                h.hop_size, h.win_size, h.fmin,
                                h.fmax_for_loss)  # 1024, 80, 24000, 240,1024
                        y_h_hat_mel_source = mel_spectrogram(
                                y_source.squeeze(1), h.n_fft, h.num_mels, h.sampling_rate,
                                h.hop_size, h.win_size, h.fmin,
                                h.fmax_for_loss)  # 1024, 80, 24000, 240,1024

                        mel_error_source = F.l1_loss(y_mel[:bs], y_h_hat_mel_source).item()
                        mel_error_mixture = F.l1_loss(y_mel[:bs], y_h_hat_mel_mix).item()

                    print(
                        'Steps : {:d}, Gen Loss Total : {:4.3f}, loss kl source: {:4.3f}, loss kl posterior: {:4.3f}, snr reconstruction: {:4.3f}, snr separation: {:4.3f}, mel_error_source: {:4.3f}, mel_error_mixture: {:4.3f}, s/b : {:4.3f}'.
                        format(steps, loss_gen_all.item(), loss_kl_source.item(), loss_posterior_matching.item(), -loss_snr_rec.item(), -loss_snr_sep.item(), mel_error_source, mel_error_mixture, time.time() - start_b))
                # checkpointing
                ...
                # if True:
                    ...
                    sw.add_scalar("training/snr_reconstruction", -loss_snr_rec, steps)
                    sw.add_scalar("training/snr_separation", -loss_snr_sep, steps)
                ...
                # if steps % 10 == 0 and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            y_source = batch['output_source']
                            y_mix = batch['output_mix']
                            ...
                            val_psm_tot += batch['loss_posterior_matching'].clone().mean().item()
                            val_kl_source_tot += batch['loss_KLD'].clone().mean().item()
                            val_snr_rec_tot += (-batch['loss_snr_source'].clone().mean().item())
                            val_snr_sep_tot += (-batch['loss_snr_mixture'].clone().mean().item())
                            ...
                            if j <= 8:
                                ...
                                sw.add_audio('source_vocals/y_hat_{}'.format(j),
                                             y_source[0], steps, h.sampling_rate)
                                sw.add_audio('mixture_vocals/y_hat_{}'.format(j),
                                             y_mix[0], steps, h.sampling_rate)

                                sw.add_audio('gt_drums/y_{}'.format(j), y[1],
                                                steps, h.sampling_rate)
                                sw.add_audio('source_drums/y_hat_{}'.format(j),
                                             y_source[1], steps, h.sampling_rate)
                                sw.add_audio('mixture_drums/y_hat_{}'.format(j),
                                             y_mix[1], steps, h.sampling_rate)

                                sw.add_audio('gt_bass/y_{}'.format(j), y[2],
                                                steps, h.sampling_rate)
                                sw.add_audio('source_bass/y_hat_{}'.format(j),
                                             y_source[2], steps, h.sampling_rate)
                                sw.add_audio('mixture_bass/y_hat_{}'.format(j),
                                             y_mix[2], steps, h.sampling_rate)

                                sw.add_audio('gt_other/y_{}'.format(j), y[3],
                                                steps, h.sampling_rate)
                                sw.add_audio('source_other/y_hat_{}'.format(j),
                                             y_source[3], steps, h.sampling_rate)
                                sw.add_audio('mixture_other/y_hat_{}'.format(j),
                                             y_mix[3], steps, h.sampling_rate)

                        val_err = val_err_tot / (j + 1)
                        val_err_ori = val_err_self_tot / (j + 1)
                        val_psm = val_psm_tot / (j + 1)
                        val_kl_source = val_kl_source_tot / (j + 1)
                        val_snr_rec = val_snr_rec_tot / (j + 1)
                        val_snr_sep = val_snr_sep_tot / (j + 1)
                        ...
                        sw.add_scalar("validation/rec_snr", val_snr_rec, steps)
                        sw.add_scalar("validation/sep_snr", val_snr_sep, steps)
                        if not plot_gt_once:
                            plot_gt_once = True
